...
export function SongSection({ searchQuery, refresh, onEdit, onDelete }: SongSectionProps) {
  ...
  // Tính toán số dòng bị thiếu để lấp đầy (Ghost Rows)
          ...
          <TableBody>
            {loading ? (
              ...
              <>
                ...
                {/* Dòng trống (Ghost Rows) lấp đầy chỗ trống để giữ form */}
                {emptyRows > 0 && Array.from({ length: emptyRows }).map((_, index) => (
                  <TableRow key={`empty-${index}`} className="border-transparent hover:bg-transparent h-20 pointer-events-none">
