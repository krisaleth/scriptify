import { Play, Heart, Search, Loader2, Music2, Clock3, ListMusic } from 'lucide-react';
...
export function PlaylistView() {
  ...
  // --- FETCH DỮ LIỆU PLAYLIST ---
  const fetchPlaylistDetails = useCallback(async () => {
    if (!id) return;
    try {
      setIsLoading(true);
      // Gọi API lấy thông tin playlist cụ thể
      ...
      toast.error("Không thể tải thông tin Playlist này!");
    } finally {
      setIsLoading(false);
    }
  }, [id]);
  ...
  // --- FILTER BÀI HÁT TRONG PLAYLIST ---
  const filteredTracks = useMemo(() => {
    if (!playlistData?.songs) return [];
    return playlistData.songs.filter((track: any) => {
        ...
        <div className="flex flex-col md:flex-row items-end gap-8 mb-8">
          ...
          <div className="w-52 h-52 bg-secondary rounded-3xl shadow-2xl flex items-center justify-center flex-shrink-0 border border-border overflow-hidden animate-in zoom-in duration-700">
            {playlistData.imageUrl ? (
              <img 
                src={getResourceUrl(playlistData.imageUrl)} 
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 text-sm">
              <span className="text-foreground font-black uppercase italic text-sm tracking-widest">
                {playlistData.creatorName || "Khải Nguyễn"}
              </span>
              <span className="text-muted-foreground font-bold">•</span>
              <span className="text-primary font-black italic uppercase text-xs tracking-widest">{playlistData.songs?.length || 0} Tracks</span>
            </div>
          </div>
        </div>
