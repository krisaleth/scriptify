...
export function HomeView() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-black pb-40 custom-scrollbar select-none overflow-x-hidden">
      ...
      <div className="px-8">
        <div className="flex items-center justify-between mb-8">
          <h3 className="text-2xl font-black text-white italic tracking-tighter uppercase">Dành riêng cho bạn</h3>
...
// --- SUB-COMPONENTS (Sử dụng CSS chuẩn xanh sếp thích) ---
  ...
  return (
    <div className="group bg-zinc-900/20 rounded-2xl p-4 hover:bg-zinc-800/40 transition-all duration-500 flex items-center gap-5 cursor-pointer border border-white/5 hover:border-green-500/20 shadow-xl relative overflow-hidden">
      ...
      <div className="flex flex-col items-center min-w-[35px]">
        <button onClick={(e) => { e.stopPropagation(); onToggleFav(); }} className={cn("transition-all active:scale-150 p-2", isFav ? "text-green-500" : "text-zinc-700 hover:text-zinc-400")}>
          <Heart className={cn("w-6 h-6", isFav && "fill-current")} />
        </button>
        <span className={cn("text-[10px] font-black", isFav ? "text-green-500" : "text-zinc-800")}>{track.likeCount || 0}</span>
      </div>
    </div>
  );
}
...
function TrackCardVertical({ track, isFav, onPlay, onToggleFav }: any) {
  return (
    <div className="group relative bg-zinc-900/20 p-5 rounded-[2.5rem] hover:bg-zinc-800/40 transition-all duration-500 cursor-pointer border border-transparent hover:border-green-500/20 shadow-2xl overflow-hidden active:scale-95">
      ...
      <h4 className="text-white font-black truncate mb-2 text-base uppercase italic tracking-tighter group-hover:text-green-500 transition-colors">{track.title}</h4>
      <div className="flex justify-between items-center">
        ...
        <button onClick={(e) => { e.stopPropagation(); onToggleFav(); }} className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all active:scale-125 shadow-lg", isFav ? "bg-green-500/10 border-green-500/30 text-green-500" : "bg-black/40 border-white/5 text-zinc-700 hover:text-zinc-400")}>
