...
export function AlbumDetailView() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gradient-to-b from-muted/30 to-background pb-32 custom-scrollbar">
      {/* Header Section */}
      <div className="bg-gradient-to-b from-primary/10 to-transparent px-8 pt-8 pb-8">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition mb-6 group bg-secondary/50 w-fit px-4 py-1.5 rounded-full border border-border"
        >
        ...
        <div className="flex flex-col md:flex-row items-center md:items-end gap-8 max-w-6xl mx-auto">
          <div className="w-64 h-64 flex-shrink-0 shadow-2xl rounded-3xl overflow-hidden group border border-border">
            <img
          ...
          <div className="flex flex-col items-center md:items-start flex-1">
            <p className="text-[10px] font-black text-primary mb-2 uppercase tracking-[0.4em] italic">Cloud Album</p>
            <h1 className="text-5xl md:text-7xl font-black text-foreground mb-4 tracking-tighter leading-none text-center md:text-left uppercase italic">
              {album.title}
            </h1>
            
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6 font-black uppercase tracking-tight italic">
              ...
              <span className="opacity-50">•</span>
              <span className="text-primary">{tracks.length} Tracks</span>
            </div>
      ...
      <div className="px-8 mt-6">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-[40px_1fr_120px] px-4 py-2 border-b border-border text-muted-foreground text-[10px] font-black uppercase tracking-widest mb-4 italic">
            <span>#</span>
          ...
          <div className="space-y-1">
            {tracks.length > 0 ? (
              tracks.map((track, index) => (
                <div
                  key={track.id}
                  className="grid grid-cols-[40px_1fr_120px] items-center gap-4 px-4 py-3 rounded-2xl hover:bg-accent transition-all group cursor-pointer border border-transparent hover:border-border"
                ...
                >
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <div className="text-[10px] text-muted-foreground truncate font-black uppercase tracking-widest mt-0.5">
                        {track.artist?.name}
                    </div>
                  </div>

                  <div className="flex items-center justify-end gap-3 text-muted-foreground font-black text-[10px] pr-4 italic uppercase tracking-widest">
