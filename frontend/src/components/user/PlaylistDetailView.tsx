import { ArrowLeft, Play, Heart, Loader2, Music, Trash2, Globe, Lock, Plus, Search, X } from 'lucide-react';
...
interface MusicContextType {
  ...
  allSongs: any[]; // Lấy thêm kho nhạc để hỗ trợ tìm kiếm
}
...
export function PlaylistDetailView() {
  ...
  const handleAddSong = async (song: any) => {
    try {
      const res = await fetch(`${API_BASE}/playlists/${id}/songs/${song.id}`, {
        method: 'POST',
        credentials: 'include'
      });
      ...
      if (res.ok) {
        ...
        toast.error("Bài hát này có thể đã tồn tại trong playlist.");
      }
    } catch (err) {
      toast.error("Lỗi kết nối đến máy chủ Cloud!");
    }
  ...
  // Logic lọc bài hát trong kho (Loại những bài đã có trong playlist)
  const availableSongsToAdd = useMemo(() => {
    if (!allSongs || !playlist) return [];
    const query = addSearchQuery.toLowerCase();
    const existingSongIds = playlist.songs?.map((s: any) => s.id) || [];
    
    return allSongs.filter(song => 
      !existingSongIds.includes(song.id) && 
      (song.title?.toLowerCase().includes(query) || song.artist?.name?.toLowerCase().includes(query))
    );
  }, [allSongs, playlist, addSearchQuery]);
    ...
    <>
              ...
              <div className="flex items-center gap-4">
                ...
                {isOwner && (
                  <button
                    ...
                    className="w-12 h-12 rounded-full border border-border flex items-center justify-center text-muted-foreground hover:border-primary hover:text-primary transition-all group shadow-sm bg-background/50 backdrop-blur-md"
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-card border border-border w-full max-w-xl rounded-[2rem] p-6 shadow-2xl relative animate-in zoom-in-95 duration-200 flex flex-col h-[70vh]">
            
            <button 
              onClick={() => setIsAddModalOpen(false)} 
              className="absolute top-4 right-4 p-2 text-muted-foreground hover:text-destructive transition-colors bg-secondary/50 rounded-full z-10"
            >
               <X size={16} />
            </button>

            <h3 className="text-2xl font-black italic uppercase mb-6 text-foreground tracking-tighter">Thêm Giai Điệu</h3>
            ...
            <div className="relative mb-4 shrink-0 group">
              ...
              <input
                ...
                className="w-full bg-secondary/50 text-foreground placeholder-muted-foreground rounded-full py-3.5 pl-12 pr-6 focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all border border-border shadow-sm italic font-bold text-sm"
              />
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-2">
              {availableSongsToAdd.length > 0 ? (
                availableSongsToAdd.map((song) => (
                  <div key={song.id} className="flex items-center gap-4 p-2.5 rounded-xl hover:bg-secondary border border-transparent hover:border-border transition-all group">
                    <img 
                      src={getResourceUrl(song.imageUrl)} 
                      alt={song.title}
                      className="w-12 h-12 object-cover rounded-lg bg-background border border-border shadow-sm"
                      onError={(e) => e.currentTarget.src = "/assets/default-cover.png"}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="font-black text-sm uppercase italic truncate group-hover:text-primary transition-colors">{song.title}</p>
                      <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest truncate">{song.artist?.name || "Ẩn danh"}</p>
                    </div>
                    <button 
                      onClick={() => handleAddSong(song)}
                      className="rounded-full border border-primary/50 text-primary hover:bg-primary hover:text-primary-foreground transition-colors font-black italic text-[10px] px-4 py-2 uppercase tracking-widest"
                    >
                      Thêm
                    </button>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground/50">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest italic text-center">
                    {addSearchQuery ? "Không tìm thấy bài hát phù hợp." : "Tất cả bài hát đã có trong Playlist."}
                  </p>
                </div>
              )}
            </div>
